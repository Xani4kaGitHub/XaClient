package moscow.xaclient.utility.game.countermine;
...
import moscow.xaclient.systems.modules.modules.other.CounterMine;
...
import moscow.xaclient.utility.animation.types.RotationAnimation;
...
public class RageBot implements IMinecraft {
   ...
   private final EventListener<ClientPlayerTickEvent> onTick = event -> {
      ...
      if (this.aim.isEnabled()) {
         ...
         Entity targetEntity = XaClient.getInstance().getTargetManager().getCurrentTarget();
            ...
            if (this.rage.isEnabled() || !(deltaYaw > 145.0F)) {
               if (this.rage.isEnabled()) {
                  if (this.silent.isEnabled()) {
                     XaClient.getInstance().getRotationHandler().rotate(toTarget, MoveCorrection.NONE, 180.0F, 180.0F, 180.0F, RotationPriority.TO_TARGET);
                  } else {
