   ...
   public static float getFallDistance(LivingEntity target) {
      ...
      if (slot != null) {
         ...
         return !ServerUtility.isFS() && !ServerUtility.isST()
            ? 0.0F
            : (
               XaClient.getInstance().getModuleManager().getModule(Aura.class).getAttacks() % 10 == 0
                  ? 0.4F
                  : (XaClient.getInstance().getModuleManager().getModule(Aura.class).getAttacks() % 5 == 0 ? 0.2F : 0.0F)
            );
      }
   }
      ...
      if (mc.world != null && mc.player != null) {
         ...
         return mc.player.isClimbing()
            || EntityUtility.getBlock(0.0, 2.0, 0.0) != Blocks.AIR
               && (
                  XaClient.getInstance().getModuleManager().getModule(Aura.class).getAttacks() % 10 == 0
                     || XaClient.getInstance().getModuleManager().getModule(Aura.class).getAttacks() % 5 == 0
               )
            ...
            || ServerUtility.isST()
               && XaClient.getInstance().getModuleManager().getModule(Aura.class).getAttacks() % 5 == 0
               && EntityUtility.getBlock(0.0, 2.0, 0.0) != Blocks.AIR
            || XaClient.getInstance().getModuleManager().getModule(Criticals.class).canCritical();
      } else {
         return false;
      }
   }
