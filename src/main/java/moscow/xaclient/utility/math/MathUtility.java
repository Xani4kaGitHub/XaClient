package moscow.xaclient.utility.math;
...
import moscow.xaclient.utility.math.calculator.ExpressionBuilder;
...
public final class MathUtility implements IMinecraft {
   ...
   public static boolean canTraceWithBlock(double rayTraceDistance, float yaw, float pitch, Entity entity, Entity target, boolean checkBlocks) {
      if (target != null && entity != null && mc.world != null) {
         ...
         Vec3d endPos = XaClient.getInstance().getModuleManager().getModule(ElytraTarget.class).isEnabled() && target instanceof PlayerEntity player
