package moscow.xaclient.systems.modules.impl;
...
public abstract class BaseModule implements Module {
   ...
   public void setEnabled(boolean newState, boolean silent) {
      if (this.enabled != newState) {
         ...
         if (!(this instanceof MenuModule) && XaClient.getInstance().getModuleManager().getModule(Sounds.class).isEnabled() && !silent) {
            ClientSounds.MODULE
               .play(XaClient.getInstance().getModuleManager().getModule(Sounds.class).getVolume().getCurrentValue(), this.enabled ? 1.1F : 1.0F);
         }
         ...
         if (this.enabled) {
            ...
            XaClient.getInstance().getEventManager().unsubscribe(this);
            if (!silent) {
               XaClient.getInstance()
                  .getNotificationManager()
                  .addNotification(
