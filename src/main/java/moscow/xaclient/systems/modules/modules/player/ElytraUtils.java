...
public class ElytraUtils extends BaseModule {
   ...
   private final EventListener<FireworkEvent> onFirework = event -> {
      if (this.boost.isEnabled() && event.getEntity() == mc.player) {
         if (XaClient.getInstance().getTargetManager().getLivingTarget() instanceof PlayerEntity player && !ElytraPredictionSystem.isLeaving(player)) {
         }
   ...
   private double getAdvancedBoost() {
      if (XaClient.getInstance().getModuleManager().getModule(ElytraTarget.class).isDefensiveActive()) {
      }
