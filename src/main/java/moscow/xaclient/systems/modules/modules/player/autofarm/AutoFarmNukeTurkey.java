package moscow.xaclient.systems.modules.modules.player.autofarm;
...
public class AutoFarmNukeTurkey implements IMinecraft {
   ...
   public void nuke() {
         ...
         for (int x = -4; x <= 4; x++) {
            for (int z = -4; z <= 4; z++) {
               ...
               if (!(mc.player.getPos().distanceTo(Vec3d.ofCenter(pos)) > 6.0)) {
                  ...
                  AutoFarm autoFarm = XaClient.getInstance().getModuleManager().getModule(AutoFarm.class);
      ...
      if (targetBlockPos != null && targetBlock != null) {
         ...
         XaClient.getInstance().getRotationHandler().rotate(new Rotation(yaw, pitch), MoveCorrection.SILENT, 180.0F, 80.0F, 180.0F, RotationPriority.NORMAL);
         this.equipAxe();
