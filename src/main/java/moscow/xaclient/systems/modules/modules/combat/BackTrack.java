...
import moscow.xaclient.utility.mixins.BacktrackableEntity;
   ...
   private final EventListener<ClientPlayerTickEvent> updateEvent = event -> {
      for (PlayerEntity player : mc.world.getPlayers()) {
         if (player != mc.player
            ...
            && backtrackableEntity.xaclient2_0$getBackTracks().size() > 2) {
            backtrackableEntity.xaclient2_0$getBackTracks().removeFirst();
         }
      }
   };
      ...
      if (this.visual.isEnabled()) {
         ...
         for (PlayerEntity player : mc.world.getPlayers()) {
            if (player != mc.player
               ...
               List<BackTrack.Position> backTracks = backtrackable.xaclient2_0$getBackTracks();
