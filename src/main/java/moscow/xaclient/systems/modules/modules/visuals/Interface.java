...
import moscow.xaclient.systems.localization.Language;
...
public class Interface extends BaseModule {
   ...
   private final EventListener<HudRenderEvent> onHudRenderEvent = event -> {
      ...
      this.lastLang = lang;
      XaClient.getInstance().getThemeManager().setCurrentTheme(this.dark.isSelected() ? Theme.DARK : Theme.LIGHT);
   };
   ...
   private void detectLanguageByIP() {
      this.executor.submit(() -> {
         try {
            ...
            XaClient.LOGGER.error("Failed to detect language by IP", var4);
         }
      });
   }
   ...
   public static boolean glassSelected() {
      return XaClient.getInstance().getModuleManager().getModule(Interface.class).liquidGlass.isSelected();
   }

   public static float glass() {
      return XaClient.getInstance().getModuleManager().getModule(Interface.class).liquidGlassAnim.getValue();
   }
