...
import moscow.xaclient.systems.event.impl.game.StartBreakBlockEvent;
...
public class MineHelper extends BaseModule {
   ...
   private void handleLowDurability(ItemStack currentStack) {
      ...
      if (!switched && this.timer.finished(800L)) {
         XaClient.getInstance().getNotificationManager().addNotificationOther(NotificationType.ERROR, "Кирка почти сломана!", "Нет замены/опыта для починки");
         this.timer.reset();
      }
   }
   ...
   private void repairPickaxeWithBottle() {
      if (mc.player != null && mc.currentScreen == null) {
         ...
         if (!this.isValidPickaxe(pickaxe) || pickaxe.getDamage() == 0) {
            XaClient.getInstance().getNotificationManager().addNotificationOther(NotificationType.ERROR, "Ошибка", "Нет кирки или она не повреждена!");
            this.rotate = false;
   ...
   private void useExperienceBottle() {
      if (mc.player.getOffHandStack().getItem() != Items.EXPERIENCE_BOTTLE) {
         ...
         RotationHandler rotation = XaClient.getInstance().getRotationHandler();
