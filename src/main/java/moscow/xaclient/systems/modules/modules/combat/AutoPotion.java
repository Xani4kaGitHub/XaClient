...
import moscow.xaclient.utility.game.PotionUtility;
...
public class AutoPotion extends BaseModule {
   ...
   private final EventListener<InputEvent> onInput = event -> {
      if (this.task != null) {
         GuiMove guiMove = XaClient.getInstance().getModuleManager().getModule(GuiMove.class);
   ...
   private final EventListener<ClientPlayerTickEvent> onUpdateEvent = event -> {
      ...
      GuiMove guiMove = XaClient.getInstance().getModuleManager().getModule(GuiMove.class);
      if (this.task != null) {
         XaClient.getInstance().getRotationHandler().rotate(rotation, RotationPriority.USE_ITEM);
         boolean hotbar = this.task.slot instanceof HotbarSlot;
         ...
         for (SelectSetting.Value selectedValue : this.potions.getSelectedValues()) {
            ...
            if (!mc.player.hasStatusEffect(potionValue.effect) && potionValue.throwTimer.finished(2000L) && potionValue.canThrow()) {
               ...
               if (slot != null) {
                  ...
                  XaClient.getInstance().getRotationHandler().rotate(rotation, MoveCorrection.SILENT, 180.0F, 180.0F, 180.0F, RotationPriority.USE_ITEM);
