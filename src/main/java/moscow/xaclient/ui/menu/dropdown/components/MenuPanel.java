package moscow.xaclient.ui.menu.dropdown.components;
...
public class MenuPanel extends CustomComponent implements IScaledResolution {
   ...
   @Override
   public void onInit() {
      for (Module module : XaClient.getInstance()
         .getModuleManager()
         .getModules()
         .stream()
   ...
   private void updateScale(UIContext context) {
      ...
      this.sizing
         .update(
            XaClient.getInstance().getMenuScreen().isClosing()
               ? 2.0F
               : (
                  Math.abs(sr.getScaledWidth() / 2.0F - this.x) / 1500.0F * 3.0F < XaClient.getInstance().getMenuScreen().getMenuAnimation().getValue()
                     ? 1.0F
                     : 0.0F
               )
         );
   ...
   public void renderBackground(UIContext context) {
      float alpha = XaClient.getInstance().getMenuScreen().isClosing()
         ? XaClient.getInstance().getMenuScreen().getMenuAnimation().getValue()
      ...
      context.drawRoundedRect(
         ...
         Colors.getBackgroundColor().withAlpha(255.0F * (XaClient.getInstance().getThemeManager().getCurrentTheme() == Theme.DARK ? 0.55F : 0.7F))
      );
   ...
   public void renderBlur(UIContext context) {
      float alpha = XaClient.getInstance().getMenuScreen().isClosing() ? 2.0F - this.sizing.getValue() : this.sizing.getValue();
      this.scale(context);
   ...
   public void onMouseClicked(double mouseX, double mouseY, MouseButton button) {
      if (this.selectedModuleComponent != null) {
         ...
         if (GuiUtility.isHovered((double)this.x, (double)y, (double)this.width, 20.0, mouseX, mouseY) && button == MouseButton.LEFT) {
            ...
            if (XaClient.getInstance().getModuleManager().getModule(Sounds.class).isEnabled()) {
   ...
   private boolean searchCheck(ModuleComponent component) {
      if (!(XaClient.getInstance().getMenuScreen() instanceof DropDownScreen dropDownScreen)) {
         return true;
      } else {
