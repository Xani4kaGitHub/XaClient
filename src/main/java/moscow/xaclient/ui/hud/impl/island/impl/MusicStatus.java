...
import moscow.xaclient.ui.hud.impl.island.ExtandableStatus;
...
public class MusicStatus extends ExtandableStatus implements IMinecraft {
   ...
   private final EventListener<MouseScrollEvent> onMouseScroll = event -> {
      if (this.showLyrics) {
         ...
         if (island.active() == this && island.isExtended()) {
            ...
            String[] lines = XaClient.getInstance().getMusicTracker().getLyrics().split("\\n");
   ...
   public void draw(CustomDrawContext context) {
      ...
      float y = 7.0F;
      MusicTracker tracker = XaClient.getInstance().getMusicTracker();
      ...
      if (tracker.haveActiveSession() && tracker.getSession() != null) {
         ...
         Identifier trackImage = tracker.getImage() != null ? tracker.getImage() : XaClient.id("icons/music/no_image.png");
         ...
         if (extending != 0.0F && tracker.getSession() != null) {
            ...
            if (extending > 0.7F) {
               ...
               context.drawTexture(XaClient.id("icons/music/previous.png"), previous, textColor.withAlpha(255.0F - 100.0F * this.hoverPrevious.getValue()));
               ...
               context.drawTexture(
                  XaClient.id("icons/music/play.png"), pause, textColor.withAlpha(255.0F * (1.0F - anim) - 100.0F * this.hoverPause.getValue())
               );
               ...
               context.drawTexture(XaClient.id("icons/music/pause.png"), pause, textColor.withAlpha(255.0F * anim - 100.0F * this.hoverPause.getValue()));
               RenderUtility.end(context.getMatrices());
               RenderUtility.end(context.getMatrices());
               context.drawTexture(XaClient.id("icons/music/next.png"), next, textColor.withAlpha(255.0F - 100.0F * this.hoverNext.getValue()));
            }
            ...
            if (owner != null) {
               context.drawTexture(XaClient.id("icons/media/" + owner + ".png"), x + expWidth - 22.0F, y + expHeight - 21.0F, 8.0F, 8.0F, ColorRGBA.WHITE);
            }

            switch (WindowsMediaSession.getCycle()) {
               case 0:
                  context.drawTexture(XaClient.id("icons/music/repeat.png"), x + 14.0F, y + expHeight - 21.0F, 8.0F, 8.0F, textColor.withAlpha(150.0F));
                  break;
               case 1:
                  context.drawTexture(XaClient.id("icons/music/repeat.png"), x + 14.0F, y + expHeight - 21.0F, 8.0F, 8.0F, textColor);
                  break;
               case 2:
                  context.drawTexture(XaClient.id("icons/music/repeat1.png"), x + 14.0F, y + expHeight - 21.0F, 8.0F, 8.0F, textColor);
            }
            ...
            if (!tracker.getLyrics().isEmpty() && tracker.getLyrics().split("butors\\n\\n").length > 1) {
               ...
               context.drawTexture(XaClient.id("icons/music/text.png"), lyricsRect, textColor.withAlpha(255.0F - 100.0F * this.hoverLyrics.getValue()));
            }
   ...
   public boolean canShow() {
      return XaClient.getInstance().getMusicTracker().getSession() != null
         && XaClient.getInstance().getMusicTracker().haveActiveSession()
         && !XaClient.getInstance().getMusicTracker().getSession().getOwner().toLowerCase().contains("gram");
   }
   ...
   public ColorRGBA getColor() {
      return super.getColor().mix(XaClient.getInstance().getMusicTracker().getMediaColor(), 0.2F);
   }
}
