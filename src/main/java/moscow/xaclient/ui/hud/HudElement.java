...
public abstract class HudElement implements SettingsContainer, IMinecraft {
   ...
   public void update(UIContext context) {
      ...
      if (widthDelta != 0.0F) {
         for (HudElement otherElement : XaClient.getInstance().getHud().getElements()) {
      ...
      if (this.dragging) {
         ...
         if (!(this instanceof DynamicIsland)) {
            for (GridLine line : XaClient.getInstance().getHud().getGrid().getLines()) {
   ...
   public void onMouseClicked(double mouseX, double mouseY, MouseButton button) {
      if (this.isHovered(mouseX, mouseY) && this.showing) {
         if (button == MouseButton.LEFT) {
            ...
            XaClient.getInstance().getHud().getPopups().add(popup);
         }
      }
   }
   ...
   public void onMouseReleased(double mouseX, double mouseY, MouseButton button) {
      if (this.dragging && button == MouseButton.LEFT) {
         ...
         if (this.x != this.startDragX || this.y != this.startDragY) {
            XaClient.getInstance().getHud().getHistoryManager().registerMove(this, this.startDragX, this.startDragY, this.x, this.y);
         }
