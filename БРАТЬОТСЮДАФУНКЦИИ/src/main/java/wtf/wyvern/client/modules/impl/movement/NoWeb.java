...
public class NoWeb extends Module {
   ...
   private final ModeSetting mode = new ModeSetting("Режим", "Grim", "Default", "Grim");
            ...
            if (mode.is("Grim")) {
                if (!mc.options.jumpKey.isPressed()) {
                    if (mc.options.sneakKey.isPressed()) {
                        mc.player.setVelocity(mc.player.getVelocity().x, -0.2D, mc.player.getVelocity().z);
                    } else {
                        mc.player.setVelocity(mc.player.getVelocity().x, 0.0D, mc.player.getVelocity().z);
                    }
                } else {
                    mc.player.setVelocity(mc.player.getVelocity().x, 0.42D, mc.player.getVelocity().z);
                }

                if (MovingUtil.hasPlayerMovement()) {
                    MovingUtil.strafe(0.24D);
                }
                return;
            }
